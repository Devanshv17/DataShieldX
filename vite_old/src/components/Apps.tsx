...
const installedAppsData: AppData[] = [
    {
      name: 'VS Code',
...
const pendingAppsData: AppData[] = [
  // Pending Apps data
  // Add your pending apps data here
];

const globalAppsData: AppData[] = [
  {
      name: 'VS Code',
      description: 'Code editor',
      image: 'https://imgs.search.brave.com/i_x3Xj7berzbEMNffR4YncVE-AcMw4MHEn6bVCps96c/rs:fit:500:0:0/g:ce/aHR0cHM6Ly9sb2dv/dHlwLnVzL2ZpbGUv/dnMtY29kZS5zdmc.svg',
...
const AppGrid = () => {
  ...
  const handleSearchSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const filtered = getActiveSectionData().filter((card) =>
  ...
  const handleTabChange = (
    event: React.ChangeEvent<{}> | {}, // Explicitly define the event type
    newValue: string
  ) => {
    setCurr(newValue);
  ...
  const getActiveSectionData = (section: string = curr) => {
    switch (section) {
      case 'Installed':
